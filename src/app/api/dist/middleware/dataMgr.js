...
function setCoverPhoto(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (useSchema === 'albumsSchema') {
            yield schemas_1.albumsSchema.findByIdAndUpdate(id, { 'coverPhoto': photoName })
                .then(result => res.json(result))
                .catch(error => res.json({ error }));
        }
        else if (useSchema === 'myservicesSchema') {
            yield schemas_2.myservicesSchema.findByIdAndUpdate(id, { 'coverPhoto': photoName })
                .then(result => res.json(result))
                .catch(error => res.json({ error }));
        }
    });
...
function deleteFile(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (useSchema === 'albumsSchema') {
            yield schemas_1.albumsSchema.updateOne({ '_id': id }, { $unset: { coverPhoto: { $cond: { coverPhoto: photoName } } }, $pull: { gallery: photoName } })
                .then((result) => {
                try {
            ...
            yield schemas_2.myservicesSchema.updateOne({ '_id': id }, { $unset: { coverPhoto: { $cond: { coverPhoto: photoName } } }, $pull: { gallery: photoName } })
                .then((result) => {
                try {
                    fs_1.default.unlinkSync(upDir + `/${id}/` + photoName);
                }
...
function fetchGallery(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (useSchema === 'albumsSchema') {
            yield schemas_1.albumsSchema.findById(id, { "gallery": 1, 'coverPhoto': 1 })
                .then(result => res.json(result))
                .catch(error => res.json({ error }));
        }
        else if (useSchema === 'myservicesSchema') {
            yield schemas_2.myservicesSchema.findById(id, { "gallery": 1, 'coverPhoto': 1 })
                .then(result => res.json(result))
                .catch(error => res.json({ error }));
        }
    });
            ...
            if (!file.name) {
                Object.values(file).forEach(el => {
                    ...
                    file2.mv(uploadPath2, (err) => __awaiter(this, void 0, void 0, function* () {
                        if (!err) {
                            if (useSchema === 'albumsSchema') {
                                yield schemas_1.albumsSchema.findByIdAndUpdate(id, { $push: { 'gallery': file2.name } });
                            }
                            else if (useSchema === 'myservicesSchema') {
                                yield schemas_2.myservicesSchema.findByIdAndUpdate(id, { $push: { 'gallery': file2.name } });
                            }
                        }
                    }));
                });
